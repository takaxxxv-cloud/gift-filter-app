...
if uploaded_file_investors is not None:
    ...
    try:
        ...
        df_inv = pd.read_csv(uploaded_file_investors, encoding='shift_jis')
        
    # 必須列の確認（ID, fundIDに加えて、メールアドレスも必須に）
    ...
    if missing_columns_inv:
        ...
        # 2. 配布済みリストの読み込みと集計（アップロードされた場合のみ）
        gift_summary = pd.DataFrame(columns=['メールアドレス', '配布済金額']) # 空のデータを作っておく
        ...
        if uploaded_file_gifts is not None:
            try:
                ...
                df_gift = pd.read_csv(uploaded_file_gifts, encoding='shift_jis')
            ...
            if missing_columns_gift:
                ...
                # 同一人物（メアド）に複数回配っているケースを想定し、メアドごとに合計金額を算出しておく
        ...
        # --- 4. ★2つのデータを「メールアドレス」で結合（VLOOKUP）★ ---
        merged_df = pd.merge(grouped, gift_summary, on='メールアドレス', how='left')
        
        # 配布履歴がなくて空白(NaN)になっている部分を0円で埋める
        ...
        with tab1:
            ...
            if len(distribute_df) > 0:
                ...
                st.download_button(
                    label="📥 今回配布リストをダウンロード",
                    data=csv_data,
                    file_name="ギフト今回配布リスト.csv",
                    mime="text/csv",
                )
            else:
